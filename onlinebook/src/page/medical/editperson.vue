<template>
    <div class="addmedicalBox">
        ...
        <div class="personType">
            ...
            <div class="TypeboxValue"><input type="text" disabled style="background:#fff;"  placeholder="你的证件号码"  maxlength="18"  v-model="idcardNo" @blur="changeidcardNo(idcardNo)"></div>
        </div>
        <div class="personType">
        ...
        <div class="personType" style="border-bottom:none;">
            ...
            <div class="TypeboxValue"><input type="tel"  disabled style="background:#fff;"  maxlength="11" placeholder="预约电话" v-model="mobile" @blur="cheangemobile(mobile)"></div>
        </div>
        ...
        <div class="addcardbtn" @click="gotoaddcard" style="border-radius: .6rem;">
            确定
        </div>
        <div style="text-align:center; color:#f60;">就诊人提交30天后，才能预约就医服务</div>
...
export default {
    ...
    created(){
        ...
        this.getpersonmsg(this.$route.query.personid)
    },
    components: {
        Topheader,
        backthird,
    ...
    methods: {
        getpersonmsg:function(id){
            this.axios.get("member/family/selectById/" + id)
            .then(res => {
                console.log(res.data.result)
                this.username = res.data.result.name
                this.idcardNo = res.data.result.certificateNumber
                this.mobile = res.data.result.mobile
                this.faDefault = res.data.result.faDefault
                this.medicalInsuranceType =  res.data.result.medicalInsuranceType
            })
            .catch(error => {
            console.log(error);
            });
        ...
        gotoaddcard:function(){
            ...
            if(this.addCardBtn){
                this.axios({method: 'post', url: '/member/family/update ',data:datas})
                .then(res => {
