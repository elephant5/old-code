<template>
    <div class="productlistBox">
            ...
            <div class="roomsearch">
                <div class="searchMsg">
                    ...
                    <div class="search_hotel">
                        ...
                        <input type="text" name="" placeholder="搜索酒店名称" v-model="inputname" ref="inputname" @blur="getProductList"  @keypress="getProductList"  class="searchinput"></div>
                </div>
        ...
        <div class="vipsearchTypeBox" v-if="vipsearchFlag">
            ...
            <div class="productlistBox">
                <div class="productlistMsg" v-for="(item,index) in currentPageData" :key="item.shopPic"  @click="productMsgShow(index,item.orderFlag,item.shopResDto.shopItemResDtoList[0].discountRate)" :class="item.orderFlag ?'' : 'overStyle'">
                    ...
                    <div class="recommendProName">
                        ...
                        <div class="hotel_tips" v-if="sysServicetype.indexOf('_cpn') === -1 && item.hotel != null && item.shopResDto.shopItemResDtoList[0].productGroupProduct.gift"><span>{{item.shopResDto.shopItemResDtoList[0].productGroupProduct.gift |getGiftTxt}}</span></div>
                        <div class="hotel_add" v-if="sysServicetype.indexOf('_cpn') ===-1 && item.shopResDto.shop.address"><span>&nbsp;</span>{{item.shopResDto.shop.address}}</div>
                        ...
                        <span class="businesspro_Icon" :class="{
                                ...
                                'businesspro_Icon_medical': sysServicetype == 'medical',
...
export default {
    ...
    created() {
        ...
        // 处理专区绕过首页进入搜索页列表页
        if(getChannge("unitId") && getChannge("productId")){
            //清除缓存
    ...
    methods: {
        ...
        productMsgShow(index,orderFlag,discountrate){
            if(orderFlag){
                ...
                if(this.sysServicetype == 'medical'){
                    this.axios.post('/mars/product/detail',qs.stringify({productGroupProductId: this.articles_array[index].shopResDto.shopItemResDtoList[0].productGroupProduct.id, giftCodeId: this.$store.state.unitId}))
                .then((res)=>{
                    console.log(res.data.result)
                     var reservOrderVo = {
                        shopChannelId:res.data.result.productDetail.shopProtocol.channelId,
                        giftType:res.data.result.productDetail.productGroupProduct.gift,
                        shopItemName:this.currentPageData[index].shopResDto.shopItemResDtoList[index].shopItem.name,
                        accoNedds:this.currentPageData[index].shopResDto.shopItemResDtoList[index].shopItem.needs,
                        accoAddon:this.currentPageData[index].shopResDto.shopItemResDtoList[index].shopItem.addon,
                        hotelName:this.currentPageData[index].hotel.nameCh,
                        discountAmount:this.currentPageData[index].shopResDto.shopItemResDtoList[index].discountRate,
                        totalAmount:this.currentPageData[index].shopResDto.shopItemResDtoList[index].shopPriceRule,
                        shopId:this.currentPageData[index].shopResDto.shop.id,
                        shopItemId:res.data.result.productDetail.shopItem.id,
                        goodsId:res.data.result.productDetail.goodsSetting.goodsId,
                        productId:res.data.result.productDetail.productGroupProduct.productId,
                        countryId:res.data.result.productDetail.shop.countryId,
                        shopName:res.data.result.productDetail.shop.name
                    }
        ...
        selectVipsearchType:function(index){
            ...
            .then(res => {
                ...
                if(res.data.code == 200){
                    ...
                    if(this.sysServicetype.indexOf('_cpn') != -1){
                        this.giftType = "CPN"
                    }else if(this.sysServicetype == 'car'){
                        this.giftType = "CPN"
                    }else{
            ...
            .catch(error => {
                ...
                this.$router.push({name:'list',params:{sysService:this.$store.state.vipIndexList[index].sysService}})
        },
       // 获取产品列表
