            ...
            <div>
              ...
              >
                <i v-if="items.surplusTimes != '0' && items.type != 'cycle_type'">
                  <i v-if="items.sysService == null? '':items.sysService.indexOf('_cpn') ===-1 || items.sysService == 'others_cpn'">预订</i>
                  <i v-else>兑换</i>
                </i>
...
export default {
  ...
  created() {
    ...
    //获取登录信息
    if(getChannge("token")){
      $cookies.set(getChannge('channel').toUpperCase() + '_loginToken', getChannge("token"),'','/', this.domainname);
    }
    // 获取商品ID
            ...
            if(this.channel == 'JD'){
              //获取用户是否是超级会员
                this.axios({
                    ...
                    data:{acId:res.data.result.acid}
                })
                .then(res => {
                    if(res.data.code == "100"){
      ...
      this.axios({
        ...
        .then(res => {
          ...
          if (res.data.code == 200) {
            ...
            if (this.channel == "JD" && getChannge("prjCode") == 'jd-10891' &&  this.myVipmsgList.vipExpirationDate) {
              this.$store.commit('getGoodsId',getChannge("prjCode"))
              var actCode;
              for (var i = 0; i < res.data.result.length; i++) {
                if (res.data.result[i].goodsId == "10891") {//10891
                  this.$store.commit("getActCode", res.data.result[i].actCode);
                  actCode = res.data.result[i].actCode;
                  break;
                }
              }
              if (actCode) {
                window.location.href ="/vip?channel=" + this.channel + "&actCode=" + actCode;
              }
            } else {
              ...
              for (var i = 0; this.giftList.length > i; i++) {
                ...
                if (res.data.result[i].goodsId == "10891") {
                    this.$store.commit("getActCode", res.data.result[i].actCode);
                    actCode = res.data.result[i].actCode
                    break
                }
