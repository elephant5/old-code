...
import { message, Modal, Collapse, Form, Badge, Divider, Select, Input, Button, Tabs, span, InputNumber, Icon, Checkbox, DatePicker, Tag, Radio } from 'antd';
import { connect } from 'react-redux';
import { bindActionCreators } from 'redux';
import * as actions from '../../../store/goods/action';
import locale from 'antd/lib/date-picker/locale/zh_CN';
import GoodsGroupList from './list'
import GoodsGroupFilter from './filter'
...
@Form.create()
class AddGroupInfo extends Component {
    constructor(props) {
        super(props);
        this.state = {
            goodsId: null,
    ...
    componentDidMount() {
        const { shopTypeList } = this.props;
        this.setState({
            shopTypeList: shopTypeList
        })
    }

    resetInfo = (giftTypeList, serviceTypeList) => {
    ...
    chooseShopType = (params) => {
        this.props.actions.selectListByShopType(params.code).then((data) => {
        ...
        this.props.actions.selectGiftByShopType(params.code).then((data) => {
            const { goods } = this.props;
    ...
    render() {
        const { onEvent, goodsId, sysDict = [], onCancel } = this.props;
        const { getFieldDecorator } = this.props.form;
        const { data, giftTypeList, serviceTypeList, shopTypeList, groupDetail, isShow, useLimitId, cycleShow, strType } = this.state;
        return (
            <Modal
                title="新增产品组"
                visible={true}
                footer={null} width={'40%'} onCancel={() => onEvent('displayDiv')}
            >
                <Form onSubmit={this.handleSubmit}>
                    <Form.Item label="产品组名称：" labelCol={{ span: 5 }} wrapperCol={{ span: 18 }}>
                        {getFieldDecorator('name', { rules: [{ required: true, message: '请输入增产品组名称!' }] })(
                            <Input maxLength={20} />
                        )}
                    ...
                    <Form.Item style={{ marginTop: '20px' }} label="内部简称：" labelCol={{ span: 5 }} wrapperCol={{ span: 18 }}>
                        {getFieldDecorator('shortName', {
                            rules: [{ required: true, message: '请输入内部简称' }]
                        })(<Input />
                        )}
                        ...
                        {getFieldDecorator('shopType', {})(
                            <Radio.Group buttonStyle="solid">
                                {shopTypeList && shopTypeList.map(item => (
                                    <Radio.Button value={item.code} onClick={() => this.chooseShopType(item)}>{item.name}</Radio.Button>
                                    // <CheckableTag {...this.props} style={{ border: '1px solid #d9d9d9' }} checked={item.checked} onChange={() => this.chooseShopType(item)} >{item.name}</CheckableTag>
                                ))}
                            </Radio.Group>
                        )}
                    </Form.Item>
                    ...
                    <Form.Item verticalGap={1} style={{ marginTop: '20px' }} label="使用限制" labelCol={{ span: 5 }} wrapperCol={{ span: 18 }} direction="horizontal" indicatorGap >
                        {getFieldDecorator('useLimitId', { rules: [{ required: true, message: '请选择使用限制!' }] })
                       ...
                       {isShow && <span className="ant-form-text" style={{ marginLeft: '100px;' }}>{sysDict.map(item => {
                                if (item.value === useLimitId) {
                                    return item.label + ":";
                                }
                            })} </span>}
                        {isShow && getFieldDecorator('useNum', {})(<InputNumber step={1} min={0}
                            ...
                            <span>
                                ...
                                {getFieldDecorator('useNum', {})(
                                    <InputNumber style={{ width: '30%' }} step={1} min={0}
                                        formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                        parser={value => value.replace(/\s?|(,*)/g, '')} />)}
                                {getFieldDecorator('useType', { initialValue: '0' })(
                                ...
                                {getFieldDecorator('cycleTime', {})(
                                    <InputNumber style={{ width: '30%' }} step={1} min={0}
                                        formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                        parser={value => value.replace(/\s?|(,*)/g, '')} />)}
                                {getFieldDecorator('cycleType', { initialValue: '2' })(
                    ...
                    <Form.Item style={{ marginTop: '20px' }} label="折扣比例：" labelCol={{ span: 5 }} wrapperCol={{ span: 18 }} >
                        {getFieldDecorator('discountRate', {initialValue : this.state.countryValue})(
                    ...
                    <Form.Item label="预约限制" labelCol={{ span: 5 }} wrapperCol={{ span: 18 }} >
                        <span >最少提前</span>&nbsp;&nbsp;
                        {getFieldDecorator('minBookDays', {})(<InputNumber step={1} min={0}
                            formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                            parser={value => value.replace(/\s?|(,*)/g, '')} />)} &nbsp;&nbsp;天
                        <span style={{ marginLeft: '20px' }}>最多提前</span>&nbsp;&nbsp;
                        {getFieldDecorator('maxBookDays', {})(<InputNumber step={1} min={0}
                                formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                parser={value => value.replace(/\s?|(,*)/g, '')} />)} &nbsp;&nbsp;天
