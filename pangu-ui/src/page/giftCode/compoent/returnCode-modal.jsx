...
@Form.create()
class ReturnActCodeModal extends Component {
    constructor(props) {
        super(props);
        this.state = {
            checkCodesRes: null,
    ...
    render() {
        const { returnCodeVisible, returnCodeCancel, returnCodeDistroy, returnCodeOkLoading } = this.props;
        const { checkCodesRes } = this.state;
        const { getFieldDecorator } = this.props.form;
        return (
            <Modal
                title="退货激活码"
                visible={returnCodeVisible}
                // onOk={this.handleSubmit}
                onCancel={returnCodeCancel}
                footer={null}
                destroyOnClose={returnCodeDistroy}
                width={800}
            // cancelText="取消"
            // okText="确定"
            >
                    ...
                    {checkCodesRes &&
                        <div>
                            <Form.Item label="检测报告:">
                                <p style={{ fontSize: 12, margin: 0 }}>共提交{checkCodesRes.allCodes.length}个激活码，其中 <span style={{ color: "red" }}>{checkCodesRes.returnCodes.length + checkCodesRes.notExistCodes.length + checkCodesRes.obsoleteCodes.length}</span> 个状态有误， <span style={{ color: "blue" }}>{checkCodesRes.allCodes.length - checkCodesRes.returnCodes.length - checkCodesRes.notExistCodes.length - checkCodesRes.obsoleteCodes.length}</span> 个可退货</p>
                                {checkCodesRes.commonGoods == false && <p style={{ fontSize: 12, margin: 0, color: "red" }}>激活码隶属于多个商品，一次只可以退货同一商品的激活码</p>}
                                ...
                                {/* {checkCodesRes.notExistCodes && checkCodesRes.notExistCodes.map(item => (<p>{item.actCode}  不存在</p>))}
                                {checkCodesRes.returnCodes && checkCodesRes.returnCodes.map(item => (<p>{item.actCode}  已退货</p>))} */}
                            </Form.Item>
                            <Form.Item label="备注说明">
                                {getFieldDecorator('returnRemarks', {})(
                                    <TextArea autosize={{ minRows: 4, maxRows: 8 }} placeholder="备注说明" />
                                )}
                            </Form.Item>
                            {checkCodesRes.commonGoods == true && checkCodesRes.allCodes.length - checkCodesRes.returnCodes.length - checkCodesRes.notExistCodes.length - checkCodesRes.obsoleteCodes.length > 0 && <Form.Item style={{ textAlign: "center" }}>
                                <Button type="primary" onClick={this.handleSubmit} loading={returnCodeOkLoading}>确认退货</Button>
                            </Form.Item>}
                        </div>
                    }
                </Form>
...
export default ReturnActCodeModal;