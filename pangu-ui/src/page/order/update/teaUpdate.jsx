...
import { Form, Input, Button, Select, LocaleProvider, Radio, Col, Checkbox, Cascader, DatePicker, Tag, Modal ,message } from 'antd';
...
@Form.create()
class TeaUpdate extends Component {
  constructor(props) {
    super(props);
    this.lastFetchId = 0;
    this.state = {
  ...
  componentDidMount() {
    ...
    if (reservOrderInfo.dispensing !== 1) {
      ...
      this.setState({ resourceList: resourceList, groupProductList: tempgroupProductList, checked: reservOrderInfo.dispensing === 1 ? true : false });
    }
    let timeShow = [];
    let start = shopItem.openTime ? shopItem.openTime.split(":")[0] : 13;
    let end = shopItem.openTime ? shopItem.closeTime.split(":")[0] : 17;
    ...
    this.setState({ timeShow: timeShow, shopItem: shopItem, groupProductList: GoodsGroupListRes[0].groupProductList });
  }
  componentWillReceiveProps(nextProps) {

    const { newShopDetail, onEvent, reservOrderInfo } = nextProps;
    ...
    if (newShopDetail.shop && this.state.isshopDetail) {
      ...
      start = shopItem.openTime ? shopItem.openTime.split(":")[0] : 13;
      end = shopItem.openTime ? shopItem.closeTime.split(":")[0] : 17;
      let startTail = shopItem.openTime ? shopItem.openTime.split(":")[1].trim() : 0;
      let endTail = shopItem.closeTime ? shopItem.closeTime.split(":")[1].trim() : 0;
      for (let i = start; i <= end; i++) {
        let tempValue = "00";
  ...
  checkProduct = e => {
    ...
    let timeShow = [];
    let start = shopItem.openTime ? shopItem.openTime.split(":")[0] : 13;
    let end = shopItem.closeTime ? shopItem.closeTime.split(":")[0] : 17;
    let startTail = shopItem.openTime ? shopItem.openTime.split(":")[1].trim() : 0;
    let endTail = shopItem.closeTime ? shopItem.closeTime.split(":")[1].trim() : 0;
    for (let i = start; i <= end; i++) {
      let tempValue = "00";
  ...
  render() {
    ...
    const { resourceList, rooms, deparDateList, groupProductList, timeShow,stopReason } = this.state;
                ...
                {timeShow &&
                  timeShow.map(item => (
                    <Option key={item.value} value={item.value}>{item.name}
                            ...
                            {(reservOrderInfo.serviceType === '下午茶') && getFieldDecorator('giftPeopleNum', { initialValue: reservOrderInfo.giftPeopleNum })
            (
