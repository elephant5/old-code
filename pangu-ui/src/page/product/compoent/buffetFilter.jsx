...
const { Option } = Select;
@Form.create()
class BuffetFilter extends Component {
    // 查询
    handleSubmit = e => {
        e.preventDefault();
 ...
 */
    ...
    getForm = () => {
        ...
        formProps.service = getFieldDecorator('service', {
            initialValue: ['自助餐'],rules: [{required:true,message:'请选择资源类型'}]
        });
    ...
    render() {
        ...
        return (
            <div className="c-filter newseachBox">
                <Form layout="inline" onSubmit={this.handleSubmit}>
                    <Form.Item label="资源类型"  style={{ width: '340px' }}>
                        {formProps.service(
                            <Select style={{ width: 220 }} mode='multiple' showSearch
                    ...
                    <Form.Item label="权益类型" style={{ width: '340px' }}>
                        {formProps.gift(
                            <Select style={{ width: 220 }} mode='multiple' showSearch
                                filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}>
                                {/* <Option value="">全部</Option> */}
